...
得益于Mineru出色的PDF解析能力，在实际生产中如果使用了RAGFLOW来管理知识库，会需要测试Mineru解析器,但是官方只提供了Http或者MCP请求本地部署的Mineru，Mineru的VLM解析也需要一定的算力，繁琐的部署和硬件成本会给想尝试Mineru解析器的开发者造成一定的阻碍，而mineru提供了免费的额度可以调用官网的API来进行解析，所以开发这个插件。
